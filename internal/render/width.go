...
func runeWidth(r rune) int {
	...
	case r <= 0x115F, // 한글 자모
		...
		// 교통·지도. 이 블록에는 텍스트 표현이 기본인 글자가 섞여 있다
		// (🛠 U+1F6E0 은 VS16 없이는 1칸). 통째로 넣으면 그것들이 2칸으로
		// 세어져 크레딧이 불필요하게 한 줄 내려간다 — 2600-27BF 를 통째로
		// 넣지 않은 것과 같은 이유다. Emoji_Presentation 구간만 집고,
		// 나머지는 VS16 이 붙었을 때 승격되게 둔다.
		r >= 0x1F680 && r <= 0x1F6C5, // 🚀 … 🛅
		r == 0x1F6CC,                 // 🛌
		r >= 0x1F6D0 && r <= 0x1F6D2,
		r >= 0x1F6D5 && r <= 0x1F6D7,
