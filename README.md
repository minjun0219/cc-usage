...
Sonnet · ctx 40% · 5h 70% (1h 20m→18:00) · 7d 45% (2d 4h) · 💳 $11.60 / $50.00
```

한도가 소진되면 크레딧이 제 줄로 내려갑니다.

```
~/dev/workspaces/cc-usage · ⎇ main +3 !5 ⇡1
Sonnet · ctx 40% · 5h 0% (1h 20m→18:00) · 7d 45% (2d 4h)
...
- **크레딧 줄은 쓴 크레딧이 0이 아니면 항상 나옵니다.** `$0.00`은 자리만 먹고 아무것도 말하지 않으므로 0이면 줄 자체를 내지 않습니다. 평소에는 **상태 줄 끝에 붙어** 두 줄로 끝나고, 강조가 붙는 상태(크레딧 소진 중 · 한도 소진 · 조회 중)에서만 **제 줄로 내려갑니다**. 문장이 길어져 줄이 감기는 걸 막기도 하고, 줄이 하나 느는 것 자체가 신호이기도 합니다. 크레딧이 비활성이거나 응답에 없을 때도 줄이 나오지 않습니다.
