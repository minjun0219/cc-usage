...
Sonnet · ctx 40% · 5h 70% (1h 20m→18:00) · 💳 $38.40 ($50.00)
```
...
```
~/dev/workspaces/cc-usage · ⎇ main +3 !5 ⇡1
Sonnet · ctx 40% · 7d 25% (2d 4h) · 5h 0% (1h 20m→18:00)
...
- **크레딧 금액은 "남은 값"입니다.** 5h·7d가 남은 비율인데 크레딧만 쓴 금액이면 방향이 엇갈려 뒤집어 읽게 됩니다. 색도 한도 창과 같은 규칙으로 골라서, 90%를 쓴 상태가 흐린 회색으로 조용히 지나가지 않습니다. 한도는 괄호로 감싸 흐리게 둡니다(`$9.83 ($100.00)`) — 한도 창의 `86% (3h 29m→14:40)`과 같은 꼴이라, 값 뒤의 괄호는 부가 정보라는 규칙이 줄 전체에서 한결같습니다.
